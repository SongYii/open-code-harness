// Package launcher owns the shared CLI pipeline used by och and SDK launchers.
package launcher
...
func Run(ctx context.Context, arguments []string, streams Streams, extensions Extensions) error {
	...
	if flags.NArg() != 0 {
		return fmt.Errorf("och: unexpected positional arguments")
	}
	...
	if *serveACP {
		fmt.Fprintf(streams.Err, "och: acp v1 on stdin/stdout; workspace=%s\n", config.WorkspaceRoot)
		return errors.Join(assembly.ServeACP(ctx, streams.In, streams.Out), assembly.Close())
	}
	fmt.Fprintf(streams.Out, "och: ready; workspace=%s database=%s runtime=%s\n", config.WorkspaceRoot, config.DatabasePath, config.RuntimeID)
	var workErr error
	select {
	case <-ctx.Done():
	case <-assembly.Done():
		workErr = fmt.Errorf("och: runtime lease lost; restart required")
	}
	fmt.Fprintln(streams.Out, "och: shutting down")
	return errors.Join(workErr, assembly.Close())
}
