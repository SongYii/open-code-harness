...
func TestMutationPostPublicationChangeReturnsStale(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if !tools.IsCode(err, tools.CodeFilesystemStaleVersion) {
			...
			if _, err := files.Write(context.Background(), abs, []byte("blind next write"), guard); !tools.IsCode(err, tools.CodeFilesystemStaleVersion) {
