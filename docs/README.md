...
| Accepted | Plan | [Context Engine implementation plan](superpowers/plans/2026-09-01-context-engine.md) | Frozen sixteen-task sequence: six pure `contextengine` package tasks, Domain commands/events, engine/Adapter request-purpose plumbing, pre-turn and mid-turn/overflow Application orchestration, manual `compact-session`, a dedicated concurrency race matrix, SQLite migration 5 with hash-chain-verified checkpoint storage, crash recovery, composition/ACP/transcript wiring, and a final evidence/benchmark/documentation task; completion claims require an evidence ledger, not checkbox state; no Chinese reading copy, matching the three most recent prior plans' precedent |
