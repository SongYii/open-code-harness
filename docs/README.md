...
| Accepted | Plan | [Minimal ACP-native client implementation plan](superpowers/plans/2026-08-30-acp-native-client.md) | Frozen six-task sequence, not yet executed: wire transport, session lifecycle client, trajectory reducer, permission-prompt handling, the `cmd/acp-client` binary and real interoperability proof, and contract publication; no Chinese reading copy, matching the most recent prior plan's precedent |
...
5. **Tool Runtime, Policy, and minimal workspace tools** — designed, implemented, and verified; not GA. Application-owned Step loop, pure Policy Decide, and four builtin workspace tools behind ports; not a plugin kernel. `exec` now has real OS-level confinement — bwrap and a cgroup v2 memory quota on Linux, Seatbelt and RLIMIT_AS on macOS — with a fail-closed `composition.Open` gate and a named, logged `AllowUnsandboxedExec` escape hatch when neither backend is available (accepted regression on Windows, which has none in this slice).
