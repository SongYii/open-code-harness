...
| Draft | Normative design | [Exec sandboxing and resource quotas](superpowers/specs/2026-08-30-exec-sandboxing-resource-quotas-design.md) | Pending human review: bubblewrap + cgroup v2 memory quota on Linux, Seatbelt + RLIMIT_AS on macOS, fail-closed availability check at composition time, structured per-effect enforcement reporting; Windows sandboxing named as an open trade-off, not resolved |
| Draft | Reading copy | [Exec 沙箱与资源配额设计（中文摘要）](superpowers/specs/2026-08-30-exec-sandboxing-resource-quotas-design.zh-CN.md) | 与 exec 沙箱与资源配额设计同步的中文摘要阅读版 |
