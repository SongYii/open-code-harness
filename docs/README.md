...
| Draft | Normative design | [Minimal ACP-native client](superpowers/specs/2026-08-30-acp-native-client-design.md) | Pending human review: a new `internal/client/acp` + `cmd/acp-client`, own-framing transport, live-`session/update`-only trajectory reducer, declined `fs`/`terminal` capabilities, plain line-oriented terminal output — step 2 of the client-surface-and-security sequencing decision |
| Draft | Reading copy | [最小 ACP 原生客户端设计（中文摘要）](superpowers/specs/2026-08-30-acp-native-client-design.zh-CN.md) | 与最小 ACP 原生客户端设计同步的中文摘要阅读版 |
