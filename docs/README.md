...
| Proposed | Normative design | [Context Engine: budgeted history, durable compaction, and recovery](superpowers/specs/2026-09-01-context-engine-design.md) | Draft milestone 8 contract for one model-neutral Context Engine: routed token budgets, bounded two-pass event projection, safe Turn/Step cuts, rolling and deterministic-reset checkpoints, manual/overflow flows, exact request recording, and rebuildable memory/SQLite checkpoint indexes; pending review before an implementation plan |
| Proposed | Reading copy | [Context Engine：预算化历史、持久压缩与恢复](superpowers/specs/2026-09-01-context-engine-design.zh-CN.md) | 与 Context Engine 设计草案同步的中文语义阅读版；英文规范源获批前不授权实施 |
