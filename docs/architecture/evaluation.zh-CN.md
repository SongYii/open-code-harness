...
提示词身份按版本新增，绝不原地修改。旧配置仍解析到冻结的 `och_quality_judge_v1`；新配置可以显式选择 `och_quality_judge_v2`。v2 把容易误解的字段改名为 `unresolvedContradictoryEvidence`：如果审计事实已经足以推翻虚假的成功声明，这属于能够定案的普通引用，可以判 Fail；只有确实无法决定 Pass 或 Fail 的冲突才进入新字段并强制降级为 Indeterminate。v2 同时明确，给出的证据包就是本次判断的权威范围，`truncated=false` 的记录无论多短都视为按原样完整。每个版本分别严格解码，因此 v2 收到旧的歧义字段会拒绝，而不是猜测模型想表达什么。

新的语义题库可以选择 `labelReviewPolicy=evidence-v1`。启用后，每个可信标签都必须用带类型的判断事实引用具体证据路径中的逐字片段，并写出会改变标签的反例；每种证据角色都必须被引用。Pass 必须包含任务、完成和验证事实，Fail 必须包含任务和违规事实，Indeterminate 必须包含任务和无法定案的事实。该策略仅为兼容旧题库而可选；未声明策略却携带审查对象，或声明未知策略，都会被拒绝。这些检查让人工标签的依据可以审查并进入摘要，但不声称机械结构能够代替语义判断。
...
方差信号在**单次运行内**，针对一个 Cell —— Scenario × Subject × Executor 三元组 —— 的各次重复计算。分组以冻结的身份摘要为键，而不是以文档 id 为键，因此一个改了名但内容相同的文档不会把一组拆开，一个同名但被编辑过的文档也不会悄悄混进来。任何身份摘要不一致的 Attempt 是硬错误，绝不合并。

每个 Cell 的报告块携带两个分母（全部终态 Attempt，以及其中可评估的那些）、每种判定的计数、按重复序号排列的全部数字分、作为**极差**的数字散度，以及作为可评估重复中众数占比的判定稳定性。

没有 `cellVerdict` 字段，也不会把重复归约成单个分数。想要 yes/no 的消费方自己决定规则并写明。将来的具名**决策规则**可以把答案发布在分布**旁边**，但绝不能**取代**分布。目前一个也没做，因为还没有任何车道在问"这个 Cell 过了吗"；`mean`/`median` 则被直接拒绝，因为它们抹掉了 5/5 和 4/5 的区别。

### 两个可靠性字段，权力不同

可靠性是两个字段而不是一个布尔值，因为消费方会在布尔值上分支，而不会去读旁边的原因字符串。

| 字段 | 它是什么 | 它能做什么 |
| --- | --- | --- |
| `evaluableEnough` | 结构性事实：对计数做算术，不需要校准即可确定。当可评估重复低于策略声明的下限、或根本没有任何可判定的重复时为假。 | 硬规则。不满足它的 Cell 在任何车道、任何成熟度下都不得报为通过，也不得被钉为基线或用作配对臂。 |
| `exceedsDeclaredLimits` | 阈值判断：散度超过声明的上限，或稳定性低于声明的下限。随身携带策略的 `calibration` 状态。 | 在限值未校准期间只作参考。它无论如何都会被发布，但在限值能引用产出它们的那次运行之前，不得把一个 Cell 从通过改写成不通过。 |

对比集合中的框架 —— inspect_ai、terminal-bench、evals、vitest-evals、Maka —— 没有一个有第二类闸门。这是关于阈值那一半的事实；结构性那一半有寻常的对应物（`n` 不足、A/B 判为不确定、inspect_ai 在空聚合时返回 `NaN`）。本契约刻意比 inspect_ai 更严格的地方在于：从单个样本根本不报散度，而不是从中返回 `0` —— 那等于在什么都没测到的地方声称完美精度。

`MayBeReadAsAResult` 是两个消费方共享的唯一规则：不满足 `evaluableEnough` 的 Cell 无条件拒绝，而限值突破只在限值已校准时才拒绝。因此在未校准的策略下，一条很宽的臂是**被披露而不是被丢弃**的 —— 基线会把它连同自己的散度一起记下来，配对比较会返回标记为 `withinNoise` 的差值，也就是"这次运行分不出这两条臂"这句诚实的话。

### 派生的可靠性读数

在两个及以上可评估重复时，报告块还会发布可评估的通过计数与可评估分母、是否至少一次通过、以及是否全部通过。这些完全不需要校准过的阈值，因此是这个块里读者今天就能信任的部分。

它们在可评估重复少于两个时被刻意省略：只剩一个幸存者时，"全部通过"和"至少一次通过"是同一句话。

单个 Cell 的"k 次里至少一次通过"是 `at_least(1)`，**不是** `pass@k` —— 后者是 Chen 等人的数据集级无偏估计器，发布出去的字段名不得借用这个名字；有一条测试守着它，否则第一次公开对比就会是不诚实的。
...
普通 pull request CI 只以确定性验证器卡门禁。**任何方差信号都不会改变退出码**，并且有一条测试断言：在挂上一份没有任何 Cell 能满足的策略前后，退出码完全相同。

确定性车道不接受方差策略。fixture 车道跑两次的差异是确定性缺陷而不是噪声，那里有用的检查是 Bazel `--runs_per_test` 那种形状 —— 跑两次、结果必须一致 —— 它不需要这里的任何机器。散度甚至当不了那个检查的信号：`NumericScore` 只在 judge 路径上被赋值，所以 fixture 车道上散度按构造恒为 `0`，"散度必须恰好为 0"这条规则会无条件通过。
