...
覆盖 assistant、工具、压缩恢复的 COMMIT 前后，但不代表所有启动切点或
多候选会话恢复都已有强杀覆盖。
...
续约往返。一个续租 worker 配合独立 watchdog，按最后成功确认时间
计时，SQLite mutex 阻塞不能拖延 fencing。失租对本实例永久生效，
迟到的成功不能恢复接单，必须由新进程获取租约并调和。Application
仍在每次追加读取 `AuthoritySource`。详见[启动扩展合同](startup-extensibility.zh-CN.md)。

## 关停与导出器归属

`Admit` 在同一锁内检查就绪与登记操作，组合根的受管门面保留登记
直到应用清理完成。`Drain` 停止准入并取消、等待操作，期间继续心跳
以便提交终态。随后 `Shutdown` 在调用方界限内等待循环，并
通过使租约过期来释放——更新精确匹配持有的 runtime ID 与 fencing
令牌，使过期宿主绝不能释放后继者的租约（Pi 规则）。后台导出器只
在就绪后按有界节奏启动，工作被取消时停止。并发关闭幂等；超时不
主动释放租约、不关闭可能仍在使用的 store，停止续租且不恢复接单。
调用方必须终止旧进程再启动后继；自然租约过期规则不变。
...
- 自动模型或工具重试；`retryOfTurnID` 血统记录属于 Application
  命令层。
- ACP 与 TUI 面。
- GA 阻塞项：不是所有 Launch／多候选会话恢复边界都有进程级 kill 覆盖；心跳证据是确定性
