...
改了两件事。每一个破坏性文件操作现在都带着一个**守卫**——一个关于「我预期会看到什么状态」的显式承诺；并且发布是**原子的**，所以目标文件绝不会被留成写了一半的样子。

「原子」不等于「报了失败就是什么都没发生」，这个区别写在这里而不是藏起来：校验发生在 rename **之后**，所以 `Write` 可能在目标已经改变的情况下返回错误。见[发布之后的失败](#发布之后的失败)。
...
校验发生在 rename 之后，所以这是一条能走到、而且是刻意保留的路径：`Write` 返回 `fs_stale_version`，而目标文件已经被替换了。触发条件是另一个写入者在我们 rename 和我们校验之间碰了这个目标。

在那里返回错误是正确的答案——否则我们要返回的那个版本描述的并不是我们能背书的字节，而基于它建立的观测会在之后授权一次盲目覆盖。但这意味着**报出来的失败并不承诺目标文件没有变化。** 被承诺的东西更窄，但依然有用：目标文件绝不会只写了一半，而本会话记下的版本绝不会是它无法背书的那个。

在这种失败之后需要知道磁盘上到底是什么的调用方，只有一个诚实的动作，而那正是失败消息已经要求它做的事：重新读一遍文件。`TestAFailureAfterPublicationIsNotANonEvent` 把这条钉住，让它成为一条被记录的事实而不是一个意外。
