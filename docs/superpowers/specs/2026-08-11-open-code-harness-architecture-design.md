...
- 每个已实现模块必须同时提供通俗导读，用固定五问说明：解决什么问题、用户能看到什么、真实实现在哪里、开发中发现并修复了什么、当前仍缺少什么；只翻译技术术语不算完成。
- 证据账本保存当时的真实历史，不为迎合现状而重写；其中已过时的状态必须醒目标记为历史快照并指向当前架构。设计、计划、生产代码、实现合同、可复跑证据和双语通俗导读齐备后，模块文档才算完成。
