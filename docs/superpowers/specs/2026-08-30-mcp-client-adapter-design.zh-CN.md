...
**§5 关于重名的论断是错的，而且这个缺口能被不可信服务器触达。** 前缀并非单射：`tools.validateSpec` 允许名字里出现 `__`，因此服务器 `a` 的工具 `b__c` 与服务器 `a__b` 的工具 `c` 会得到同一个限定名 `mcp__a__b__c`——两个服务器名并不相同。原始工具名来自被本设计自身威胁模型视为不可信的服务器，而 `NewCatalog` 在 `composition.Open` 处 fail-closed，所以一个恶意服务器可以挑一个与**另一台**服务器的工具相撞的名字，让整个 harness 起不来。修订后的规则：拼接前先把两部分都清洗成 `[a-zA-Z0-9_-]`（这同时把分隔符从两部分中移除，恢复了单射性），限定名上限 64 字节，超出则截断并追加一个稳定的 8 位十六进制 FNV-1a 后缀。
