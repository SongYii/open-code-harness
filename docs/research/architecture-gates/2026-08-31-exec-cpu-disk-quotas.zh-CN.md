...
**更正（2026-08-31，在准备后续设计时发现）：** 上面这条 macOS 相关的发现是错的，错的地方不在那六个参考项目身上——而是没有先去查本项目自己已有的代码。`internal/harness/adapters/localexec/sandbox_darwin.go` 的 `beginRlimitBracket` 已经演示了一个真实、可用的技术，正好解决这一类问题：既然 `RLIMIT_AS` 会在 `fork()` 时被子进程继承，而 Go 的 `os/exec` 又没有钩子能只在 fork 和 exec 之间的子进程身上设置这个值，本项目的做法是把*自己*进程的 rlimit 围在父进程自己的 `Start()` 调用前后——先调低，拉起子进程（一个窄的、由互斥锁保护的窗口，这段时间里 harness 自己的进程也受同一个调低的上限约束），再恢复。`RLIMIT_CPU` 在 fork 时的继承方式跟 `RLIMIT_AS`完全一样；同一套围栏技术不用改动就能直接用在它身上。本文原来那条"这六个参考项目都没演示出任何东西"外部强制 macOS CPU 上限的结论，只对那六个外部项目成立——本项目自己已有的内存配额代码早就解决了这个通用问题。后面的设计用的是这个更正过的理解，不是上面那一段；那一段没有被悄悄改写，而是保留原文、挂上这条更正说明，跟本项目自己一贯"更正已合并文档"的做法一致。
